            ...
            if($_POST['action'] == 'skip'){ // No quests
                ...
                if($_POST['questTutorialId'] == 'Tutorial_09' && $session->questNum == 9){
                    if($session->qData['isFinished']){
                        ...
                        $database->query("UPDATE quests SET quest = 10, step1 =0, step2 = 0 , isFinished =1 WHERE userid = ".$session->uid."");
                        echo '{"response": {"error":false,"errorMsg":"Token invalid","data":{"html":""},"reload":true}}';
                    }
                }
                ...
                if($_POST['questTutorialId'] == 'Tutorial_10' && $session->questNum == 10){
                    if($session->qData['isFinished']){
                        ...
                        $database->query("UPDATE quests SET quest = 11, step1 =0, step2 = 0 , isFinished =1 WHERE userid = ".$session->uid."");
                        echo '{"response": {"error":false,"errorMsg":"Token invalid","data":{"html":""},"reload":true}}';
                    }
                }
        ...
        echo '
{
    ...
    case 'buyplus':
        ...
        $gold=$gol[0]['gold'];
        switch($id) {
			case 6: $cost=$config['goldClub'];break;
			...
			case 15: $cost=$config['goldClub'];break;
...
for($i=19;$i<=40;$i++){
	if($builds['f'.$i.'t']==12){
        ...
        break;
                ...
                if($_POST['activeTab'] == 'buyGold'){
                    ...
                    $html .= 'Yükleniyor..<meta http-equiv="refresh" content="0; URL=satinal.php?t=2" />
                   ';
                 ...
                 // bonus altın kapat ödev
				// if($session->qArrayW1[0] == 7 && $session->qArrayW1[1] == 1){
                        $database->query("UPDATE quests SET world1='7,1' WHERE userid = ".$session->uid."");
                 //   }
                ...
                $html .= '<div class="header">
                            <ul>
                                <li '.($_POST['activeTab'] == 'buyGold' ? 'class="active"' : '').'>
                                    <a href="satinal.php" class="tabButton buyGold" onclick="return false;">
    ...
    $usData = $database->queryFetch(
        "SELECT `id` FROM `users` WHERE `username` = '".$uName."'"
    );

    if($usData['id'] || $uName == '[ally]') {

        $cIgnore = 0;

        if($uName != '[ally]') {

            $ignoreData = $database->queryFetch(
                'SELECT COUNT(*) AS num FROM `ignore` 
                 WHERE `uignored` = '.$session->uid.' 
                 AND `uid` = '.$usData['id']
            );

            if($ignoreData){
                $cIgnore = $ignoreData['num'];
            }
        }

        if($cIgnore != 0 && $session->access != 9 && $uName != '[ally]') {

            echo '{"response":{"error":true,"errorMsg":"Bu oyuncuya mesaj gönderemezsiniz","data":[]}}';
...
break;
                    ...
                    switch($_POST['action']){
                        ...
                        case 'actionAddList':
                            if(empty($_POST['listName']) || empty($_POST['did'])){
                                echo $ajax->error('Listeye bir ad ekle'); exit();
                            }else{
                                if(strlen($_POST['listName']) > 12){
                                    echo $ajax->error('İsim listesi uzunluğu çok uzun'); exit();
                                }else{
                ...
                case 'demolishNowPopup':
                    //print_r($_POST); die();
                    $html = '<div id="demolishNowDialog"><p>Bu binanın tamamen yıkıldığının doğrulanmasına cevap verir misiniz? Burası senin köyünden bir bina.</p>
