        ...
        if (($village->resarray['f99t'] == 40 and ($t) == '26') or ($village->resarray['f99t'] == 40 and ($t) == '30') or ($village->resarray['f99t'] == 40 and ($t) == '31') or ($village->resarray['f99t'] == 40 and ($t) == '32')) {
            if ($x == 0) { // To show only one slut
                ...
                if ($village->resarray['f99'] >= 0 && $village->resarray['f99'] <= 9) {
                    $imageBuild = 'g40 ' . get_tribe() . ' g40_0 ' . get_tribe();
                }
                if ($village->resarray['f99'] >= 10 && $village->resarray['f99'] <= 29) {
                    $imageBuild = 'g40 ' . get_tribe() . ' g40_1 ' . get_tribe();
                }
                if ($village->resarray['f99'] >= 30 && $village->resarray['f99'] <= 49) {
                    $imageBuild = 'g40 ' . get_tribe() . ' g40_2 ' . get_tribe();
                }
                if ($village->resarray['f99'] >= 50 && $village->resarray['f99'] <= 79) {
                    $imageBuild = 'g40 ' . get_tribe() . ' g40_3 ' . get_tribe();
                }
                if ($village->resarray['f99'] >= 80 && $village->resarray['f99'] <= 99) {
                    $imageBuild = 'g40 ' . get_tribe() . ' g40_4 ' . get_tribe();
                }
                if ($village->resarray['f99'] == 100) {
                    $imageBuild = 'g40 ' . get_tribe() . ' g40_5 ' . get_tribe();
                }
            ...
            if ($t == 40) {
                ...
                if ($village->resarray['f' . ($t) . 't'] != 0) { //
                    $bindicate = $building->canBuild($t, $village->resarray['f' . $t . 't']);
                    $output .= '<div class="level colorLayer '
                        . (($village->resarray['f' . ($t)] == 0) ? 'good' : ($bindicate == 1 ? 'maxLevel' : $Travian->getStatus($t)))
                        . ' ' . ($database->getBuildingByField($tx, $t) ? 'underConstruction' : '')
                        . ' aid' . $t . '" title="' . $Travian->getTitle($t)
...
<script>
    ...
    window.addEvent('domready', function () {
        ...
        a.addEvents({
            ...
            'mouseup mouseleave blur touchend': function () {
