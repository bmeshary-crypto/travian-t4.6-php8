...
<div id="auction">
        ...
        <div class="boxes boxesColor gray">
            ...
            <div class="boxes-contents">
                <div class="wrapper">
                    ...
                    <div class="silver"><img class="silver"
                                             src="img/x.gif"> <?php $now = $database->getAuctionSilver($session->uid);
                        echo($session->silver); ?> / <?php echo $session->silver + $now['silver']; ?> </div>

                    <div class="filterContainer">
                        <button title="<?= HEROAC0 ?>" type="button"
    ...
    $sql = $database->query("SELECT * FROM auction WHERE finish = 0 and uid = '" . $session->uid . "' $mmm ORDER BY time ASC");
    $query = count($sql);
    ...
    $sql2 = $database->query("SELECT * FROM auction WHERE finish = 0 and uid = '" . $session->uid . "' $mmm ORDER BY time ASC");
    ...
    $outputList = '';
    if ($query == 0) {
        $outputList .= "<td colspan=\"7\" class=\"none\"><center>" . HEROAC15 . "</center></td>";
    } else {
        foreach ($sql2 as $row) {
            $id = $row["id"];
            $owner = $row["owner"];
            ...
            $outputList .= "<tr><td class=\"" . $sStyle . "\"></td><td class=\"icon" . $sStyle . "\"><img class=\"itemCategory itemCategory_" . $typeArray[$btype] . "\" src=\"img/x.gif\" title=\"" . $Travian->getItemData($btype, $type)['name'] . "||" . $Travian->getItemData($btype, $type)['title'] . "\"></td>";

            $outputList .= "<td class=\"name" . $sStyle . "\">x" . $num . " " . $Travian->getItemData($btype, $type)['name'] . "</td>";
            ...
            $outputList .= "</td>";
            $outputList .= "<td class=\"silver" . $sStyle . "\" title=\"" . round($silver / $num, 2) . "\">" . $silver . "</td>";
            ...
            if (isset($_GET['a']) && $_GET['a'] == $id) {
                ...
                $outputList .= "<?=HEROAC19?> <img title=\"silver\" class=\"silver\" src=\"img/x.gif\"> <span>" . ($silver) . "</span><br><?=HEROAC20?> ";
                if ($uid != 0) {
                ...
                $outputList .= "<span></span><br>" . HEROAC21 . "<input class=\"maxBid text\" type=\"text\" name=\"maxBid\" value=\"" . $bidvalue . "\">";
                $outputList .= "<span> (min <img title=\"silver\" class=\"silver\" src=\"img/x.gif\"> " . $minimum . ")</span>";
                $outputList .= '<td class="icon selected"><div class="submitBid"><button type="submit" class="green"><div class="button-container addHoverClick ">
                    ...
                    </div><div class="button-content">' . HEROAC22 . '</div></div></button></div></div></form></td></tr>';
            }
    ...
    $sql = $database->query("SELECT * FROM auction WHERE finish = 1 and uid = '" . $session->uid . "' ORDER BY time DESC");
    $query = count($sql);
    ...
    if ($page != "1") {
        $paginationDisplay .= '<a class="first" href="?action=bids&page=1"><img src="img/x.gif"></a> ';
    } else {
        $paginationDisplay .= '<img src="img/x.gif" class="first disabled"> ';
    }
    if ($page != "1" && $page != $lastPage) {
        $paginationDisplay .= '<a class="previous" href="?action=bids&page=' . $previous . '"><img src="img/x.gif"></a>';
    } else {
        $paginationDisplay .= '<img src="img/x.gif" class="previous disabled">';
    }
    $paginationDisplay .= $centerPages;
    if ($page != "1" && $page != $lastPage || $page == "1" && $lastPage != "1") {
        $paginationDisplay .= '<a class="next" href="?action=bids&page=' . $nextPage . '"><img src="img/x.gif"></a> ';
        $paginationDisplay .= '<a class="last" href="?action=bids&page=' . $lastPage . '"><img  src="img/x.gif"></a>';
    } else {
    ...
    $sql2 = $database->query("SELECT * FROM auction WHERE finish = 1 and uid = '" . $session->uid . "' ORDER BY time DESC $limit");
    ...
    $idd = 1;
    if ($query == 0) {
        $outputList .= "<td colspan=\"7\" class=\"none\"><center>" . HEROAC24 . "</center></td>";
    } else {
        foreach ($sql2 as $row) {
            $id = $row["id"];
            $owner = $row["owner"];
            ...
            $outputList .= "<td class=\"icon\"><img class=\"itemCategory itemCategory_" . $typeArray[$btype] . "\" src=\"img/x.gif\" title=\"" . $Travian->getItemData($btype, $type)['name'] . "||" . $Travian->getItemData($btype, $type)['title'] . "\"></td>";
            ...
            $outputList .= "<td class=\"time\">" . HEROAC25 . "</td>";
            $outputList .= "<td class=\"bid win\">" . HEROAC26 . "</td></tr>";
            $idd++;
        }
    }
    ...
    <form method="post" action="hero_auction.php">
        <h4 class="auctionEnded"><?= HEROAC27 ?></h4>
        ...
        <div>
            <button type="submit" class="green" name="del" id="del">
