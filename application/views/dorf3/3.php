<table id="warehouse" cellpadding="1" cellspacing="1">
    ...
    foreach ($session->villages as $vid) {
        ...
        $prod_crop -= $database->getUpkeep($database->getAllUnits($vid), $session->tribe, $database->getResourceLevel($vid));
		...
		<td class="max123"><span ' . ($timer1 != "00:00:00" ? 'id="timer' . $timer . '"' : '') . '>' . $timer1 . '</span></td>';
        if ($timer1 != "0:00:00") {
            $timer++;
        }
        echo '
		<td class="crop ' . $critCR . '" title="' . $crop . '/' . $maxc . '">' . $percentCr . '%</td>
		<td class="max4 lc"><span ' . ($timer2 != "00:00:00" ? 'id="timer' . $timer . '"' : '') . '>' . $timer2 . '</span></td></tr>';
        if ($timer2 != "0:00:00") {
            $timer++;
        }
    }
