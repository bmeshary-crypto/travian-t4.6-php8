    ...
    if ($data['password'] == md5($pass . mb_convert_case($session->username, MB_CASE_LOWER, "UTF-8"))) {
        ...
        if (count($data1)) {
            ...
            if ($data2['vref'] != $village->wid) {
                ...
                $database->query("DELETE FROM training where unit > 60 and `vref`='" . $village->wid . "'"); //delete workouts for bq
...
<button type="submit" value="ok" name="s1" id="btn_train" value="ok" class="textButtonV3 green">
