...
<div class="roundedCornersBox big <?php echo $bindicate == 1 ? 'headlineOnly' : ''; ?>">
    <div class="stickyImage">
        <a class="build_logo"
           onclick="return Travian.Game.iPopup(<?php echo $village->resarray['f' . $_GET['id'] . 't']; ?>, 4);"
           href="#">
            <img class="build_logo big black g<?php echo $village->resarray['f' . $_GET['id'] . 't'] == 12 ? 13 : $village->resarray['f' . $_GET['id'] . 't']; ?>"
                 alt="<?= $lang['buildings'][$village->resarray['f' . $_GET['id'] . 't']] ?>" src="img/x.gif">
        </a>
    </div>

    <?php if ($bindicate == 1) { ?>
    <h4>
        <div class="statusMessage"><span
                    class="none"><?= $lang['buildings'][$village->resarray['f' . $_GET['id'] . 't']] . ' ' . UPG0; ?></span>
        </div>
    </h4>
</div>
<?php }else{ ?>
...
<div class="upgradeButtonsContainer <?php if (MAX_LEVEL) {
    echo 'section2Enabled';
} ?>">
    <div class="section1">
        ...
        if ($bindicate == 2 || !$bindicate) {
            ...
            if ($neededtime == 0) {
                echo "<span class=\"none\">" . UPG10 . "</span>";
            } else {
                echo "<span class=\"none\">Will be available Resources in a  " . $neededtime[0] . " hour  " . $neededtime[1] . "</span>";
            }
        } else if ($bindicate == 8) {
            if ($id <= 18) {
    ...
    <div class=\"button-background\">
        ...
        if ($village->resourceSUM >= $totalres) {
            if ($session->gold >= 3 && $building->getTypeLevel(17) >= 1 && $bindicate == 7) {
                ...
                ?>
                <button type="button" class="gold exchange <?php echo $isDisabled ? 'disabled' : ''; ?>"
                        onfocus="jQuery('button', 'input[type!=hidden]', 'select').focus(); event.stopPropagation(); return false;"
                        id="button<?php echo crc32(array_sum($uprequire)); ?>">
        ...
        </div> <?php if ($bindicate == 9) {
            echo "<span class=\"none\">(Waiting)</span>";
        } ?>
    </div>
...
<div class="section2">
    <button type="button" class="<?php echo $class; ?> upgradeToMaxLevel <?php echo $disable; ?>" coins="15" title="<?php echo $lang['Build']['utomax2']; ?>||<?php echo $title;?>" <?php if(!$disable) { ?>onclick="window.location.href = 'ajax.php?cmd=upall&a=<?php echo $id;?>'; return false;"<?php } ?>>
	    <div class="button-container addHoverClick">
            ...
            <div class="button-content"><?php echo $lang['Build']['utomax1']; ?>
                <img src="img/x.gif" class="goldIcon" alt="">
                <span class="goldValue"><?php echo MAX_LEVEL_COST; ?></span>
            </div>
        </div>
    </button>
	<div class="clear"></div>
