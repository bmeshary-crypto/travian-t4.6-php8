<?php
echo '$session->goldclub = '.$session->goldclub;
echo 'count($session->villages) = '.count($session->villages);
if ($session->goldclub == 1 && count($session->villages) > 1) {
    ...
    if (isset($_GET['create']) && $session->gold > 1) {
        ...
        <table id="npc" cellpadding="1" cellspacing="1">
            ...
            if (count($routes) == 0) {
                echo "<td colspan=\"5\" class=\"none\">" . market46 . "</td></tr>";
            } else {
                foreach ($routes as $route) {
                    ?>
                    <tr>
                        <th>
                            <a href="build.php?id=<?= $id ?>&t=4&action=delRoute&routeid=<?php echo $route['id']; ?>"><img
                                        class="del" src="img/x.gif" alt="delete" title="delete"></a></th>
                        <th>
                            <?php
                            echo "" . market47 . " <a href=karte.php?d=" . $route['wid'] . "&c=" . $generator->getMapCheck($route['wid']) . ">" . $database->getVillageField($route['wid'], "name") . "</a></br>";
                            ?>
                        ...
                        <th>
                            <a href="build.php?id=<?php echo $id; ?>&t=4&action=editRoute&routeid=<?php echo $route['id']; ?>">» <?= market48 ?></a>
                        </th>
                    </tr>
                <?php }
            } ?>
        ...
        <br>
        <div class="options">
            <a class="arrow" href="build.php?s=17&t=4&create=1"> <?= market49 ?></a>
        </div>
