    ...
    if ($data['password'] == md5($pass . mb_convert_case($session->username, MB_CASE_LOWER, "UTF-8"))) {
        ...
        if (count($data1)) {
            ...
            if ($data2['vref'] != $village->wid) {
                ...
                $database->query("DELETE FROM training where unit > 60 and `vref`='" . $village->wid . "'"); //удаляем тренировки для бк
        ...
        print '
<script language="javascript">location.href = "build.php?id=' . $building->getTypeField(44) . '&confirm=yes";</script>';
    }
}
?>
...
if ($building->getTypeLevel(44) > 0) {
    ...
    ';
        ...
        if ($data['wref'] == $village->wid) {
            ...
            if ($_GET['confirm'] == '') {
                print '<p><a href="?id=' . $building->getTypeField(44) . '&confirm=yes">&raquo Change the capital</a></p>';
            } else {
