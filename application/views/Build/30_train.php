...
for ($i = ($session->tribe - 1) * 10 + 3; $i <= ($session->tribe - 1) * 10 + 6; $i++) {
    if ($i <> 3 && $i <> 13 && $i <> 14 && $i <> 53 && $technology->getTech(($i - ($session->tribe - 1) * 10))) {
        ...
        $dur = round((${'u' . $i}['time'] * ($bid30[$village->resarray['f' . $id]]['attri'] * ($building->getTypeLevel(41) >= 1 ? (1 / $bid41[$building->getTypeLevel(41)]['attri']) : 1) / 100) / SPEED * $art * $bonuses['stable']), 5);
        ...
        if ($session->gold >= 3 && $building->getTypeLevel(17) >= 1) {
            echo "&nbsp;&nbsp;<button id='button" . crc32($i) . "' type=\"button\" value=\"npc\" class=\"icon\">&nbsp;<img src=\"img/x.gif\" style=\"margin-top:6px;\" class=\"npc textButtonV3\" alt=\"npc\"></button>";
            ?>
