...
class Quest
{
    ...
    function checkQ()
    {
        ...
        if ($session->qArrayB2[0] == 14 && $session->qArrayB2[1] == 0) {
            ...
            for ($i = ($session->tribe * 10 - 9), $ii = 0; $i <= ($session->tribe * 10 - 2); $i++) {
                ...
                if ($this->getTech(($i - (($session->tribe - 1) * 10))) || $j == 1) {
                    ...
                    $ii++;
                    if (count($ABups) && $ABups[0]['tech'] == "a" . ($i - (($session->tribe - 1) * 10))) {
                        $abdata['a' . $j] += 1;
                    }
                    if ($abdata['a' . $j] != 0) {
        ...
        if ($session->qArrayE2[0] == 4 && $session->qArrayE2[1] == 0) {
            ...
            if ($addOne == 18) {
                $database->query("UPDATE quests SET economy2='4,1' WHERE userid = " . $session->uid . "");
            }
        }
        ...
        if ($session->qArrayE2[0] == 8 && $session->qArrayE2[1] == 0) {
            ...
            if ($addOne == 18) {
                $database->query("UPDATE quests SET economy2='8,1' WHERE userid = " . $session->uid . "");
            }
        }
        ...
        if ($session->qArrayE2[0] == 12 && $session->qArrayE2[1] == 0) {
            ...
            if ($addOne == 18) {
                $database->query("UPDATE quests SET economy2='12,1' WHERE userid = " . $session->uid . "");
            }
        }
    ...
    public function getQuestMonitor($quest)
    {
        ...
        if ($quest == 16) {
            ...
            if ($qArrayB1[0] < 10 && $qArrayB1[0] != 0) {
                $questIdB1 = '0' . $qArrayB1[0];
            } else {
                $questIdB1 = $qArrayB1[0];
            }
            if ($qArrayB2[0] < 10 && $qArrayB2[0] != 0) {
                $questIdB2 = '0' . $qArrayB2[0];
            } else {
                $questIdB2 = $qArrayB2[0];
            }

            if ($qArrayE1[0] < 10 && $qArrayE1[0] != 0) {
                $questIdE1 = '0' . $qArrayE1[0];
            } else {
                $questIdE1 = $qArrayE1[0];
            }
            if ($qArrayE2[0] < 10 && $qArrayE2[0] != 0) {
                $questIdE2 = '0' . $qArrayE2[0];
            } else {
                $questIdE2 = $qArrayE2[0];
            }

            if ($qArrayW1[0] < 10 && $qArrayW1[0] != 0) {
                $questIdW1 = '0' . $qArrayW1[0];
            } else {
                $questIdW1 = $qArrayW1[0];
            }
            if ($qArrayW2[0] < 10 && $qArrayW2[0] != 0) {
                $questIdW2 = '0' . $qArrayW2[0];
            } else {
                $questIdW2 = $qArrayW2[0];
            }

            $firstBid = $questIdB1 < $questIdB2 ? $questIdB1 : $questIdB2;
            $SecondBid = $questIdB1 > $questIdB2 ? $questIdB1 : $questIdB2;
            $isFinishedB1 = $questIdB1 < $questIdB2 ? 1 : 2;
            $isFinishedB2 = $questIdB1 > $questIdB2 ? 1 : 2;

            $firstEid = $questIdE1 < $questIdE2 ? $questIdE1 : $questIdE2;
            $SecondEid = $questIdE1 > $questIdE2 ? $questIdE1 : $questIdE2;
            $isFinishedE1 = $questIdE1 < $questIdE2 ? 1 : 2;
            $isFinishedE2 = $questIdE1 > $questIdE2 ? 1 : 2;

            $firstWid = $questIdW1 < $questIdW2 ? $questIdW1 : $questIdW2;
            $SecondWid = $questIdW1 > $questIdW2 ? $questIdW1 : $questIdW2;
            $isFinishedW1 = $questIdW1 < $questIdW2 ? 1 : 2;
            $isFinishedW2 = $questIdW1 > $questIdW2 ? 1 : 2;

            $checkDone = '<svg viewBox="0 0 20 19.58" class="check">
                        <path d="M7.23 12.7C3.46 9.29 1.5 7.36 1.5 7.36A6.83 6.83 0 0 0 0 6.28l7.59 13.3S11.19 7.78 20 0a8.72 8.72 0 0 0-2.24 1C13.29 3.64 9.63 9.17 7.23 12.7z"></path>
                    </svg>';

            $ret = '
                    ' . ($firstBid ? '<li data-questid="Battle_' . ($firstBid) . '" data-category="battle">' .'<a href="#" class="">' .  (${qArrayB . ($isFinishedB1)}[1] ? $checkDone : '') .'<span class="description">'. $lang['quests']['monitor']['battle'][($firstBid)] . '</span> </a></li>' : '') . '
                    ' . ($SecondBid ? '<li data-questid="Battle_' . ($SecondBid) . '" data-category="battle">' .'<a href="#" class="">' .  (${qArrayB . ($isFinishedB2)}[1] ? $checkDone : '').'<span class="description">' . $lang['quests']['monitor']['battle'][($SecondBid)] . ' </span></a></li>' : '') . '

                    ' . ($firstEid ? '<li data-questid="Economy_' . ($firstEid) . '" data-category="economy">' . '<a href="#" class="">' . (${qArrayE . ($isFinishedE1)}[1] ? $checkDone : '').'<span class="description">' . $lang['quests']['monitor']['economy'][($firstEid)] . '</span> </a></li>' : '') . '
                    ' . ($SecondEid ? '<li data-questid="Economy_' . ($SecondEid) . '" data-category="economy">' . '<a href="#" class="">' . (${qArrayE . ($isFinishedE2)}[1] ? $checkDone : '') .'<span class="description">'. $lang['quests']['monitor']['economy'][($SecondEid)] . ' </span></a></li>' : '') . '

                    ' . ($firstWid ? '<li data-questid="World_' . ($firstWid) . '" data-category="world">' . '<a href="#" class="">' . (${qArrayW . ($isFinishedW1)}[1] ? $checkDone : '') .'<span class="description">'. $lang['quests']['monitor']['world'][($firstWid)] . ' </span></a></li>' : '') . '
                    ' . ($SecondWid ? '<li data-questid="World_' . ($SecondWid) . '" data-category="world">' . '<a href="#" class="">' . (${qArrayW . ($isFinishedW2)}[1] ? $checkDone : '') .'<span class="description">'. $lang['quests']['monitor']['world'][($SecondWid)] . ' </span></a></li>' : '') . '';
        } else {
            ...
            foreach ($listo as $list) {
                $x++;
                $ret .= '<li>';
                if ($qData['step' . $x . ''] || $qData['isFinished']) {
                    $ret .= $checkDone;
                }
                $ret .= $list;
                $ret .= '</li>';
    ...
    function getimgClass($id, $isCompleted, $type = FALSE)
    {
        ...
        if ($type) {
            ...
            switch ($id) {
                case 1:
                    return 'tutorial_' . $id . '_task_image_vid' . $session->tribe;
                    break;
                case 2:
                    if ($isCompleted) {
                        return 'tutorial_' . $id . '_reward_image';
                    } else {
                        return 'tutorial_' . $id . '_task_image_vid' . $session->tribe;
                    }
                    break;
                case 14:
                case 15:
                    if ($isCompleted || $session->qData['skipped']) {
                        return 'tutorial_' . $id . '_reward_image_vid' . $session->tribe;
                    } else {
                        return 'tutorial_' . $id . '_task_image_vid' . $session->tribe;
                    }
                    break;
                default:
                    if ($isCompleted) {
                        ...
                        return 'tutorial_' . $id . '_task_image';
                    }
                    break;
            }
        }
    ...
    public function questData($id, $type = FALSE)
    {
        global $database, $session, $lang;
        if (!is_numeric($id)) {
            if ($session->qArrayB1[0] < 10) {
                $questIdB1 = '0' . $session->qArrayB1[0];
            } else {
                $questIdB1 = $session->qArrayB1[0];
            }
            if ($session->qArrayB2[0] < 10) {
                $questIdB2 = '0' . $session->qArrayB2[0];
            } else {
                $questIdB2 = $session->qArrayB2[0];
            }
            if ($session->qArrayE1[0] < 10) {
                $questIdE1 = '0' . $session->qArrayE1[0];
            } else {
                $questIdE1 = $session->qArrayE1[0];
            }
            if ($session->qArrayE2[0] < 10) {
                $questIdE2 = '0' . $session->qArrayE2[0];
            } else {
                $questIdE2 = $session->qArrayE2[0];
            }
            if ($session->qArrayW1[0] < 10) {
                $questIdW1 = '0' . $session->qArrayW1[0];
            } else {
                $questIdW1 = $session->qArrayW1[0];
            }
            if ($session->qArrayW2[0] < 10) {
                $questIdW2 = '0' . $session->qArrayW2[0];
            } else {
                $questIdW2 = $session->qArrayW2[0];
            }
            ...
            $bAll = 0;
            if ($firstBid != 0 && $SecondBid != 0) {
                $bAll++;
            }
            $eAll = 0;
            if ($firstEid != 0 && $SecondEid != 0) {
                $eAll++;
            }
            $wAll = 0;
            if ($firstWid != 0 && $SecondWid != 0) {
                $wAll++;
            }
            ...
            if (!$type) {
                ...
                if ($id == 1) {
                    $html .= '<button type="submit" class="green questButtonSkipTutorial" questbuttonskiptutorial="1" id="button72apTvUbEYHGu"><div class="button-container addHoverClick">
                                ...
                                <div class="button-content">Skip Tutorial</div></div></button>
    ...
    public function dailyReward($type)
    {
        ...
        if (is_numeric($type) && $type <= 4) {
            ...
            switch ($type) {
                case 1:
                    $reward = rand(1, 3) . ',0';
                    $prizes = "<li>" . questday5 . "</li><br><li>" . questday6 . "</li><br><li>" . questday7 . "</li><br>";
                    break;
                case 2:
                    $reward = rand(1, 5) . ',0';
                    $prizes = "<li>" . questday9 . "</li><br><li>" . questday10 . "</li><br><li>" . questday11 . "</li><br><li>" . questday12 . "</li><br><li>" . questday13 . "</li><br>";
                    break;
                case 3:
                    $reward = rand(1, 3) . ',0';
                    $prizes = "<li>" . questday15 . "</li><br><li>" . questday16 . "</li><br><li>" . questday17 . "</li><br>";
                    break;
                case 4:
                    $reward = rand(1, 3) . ',0';
                    $prizes = "<li>" . questday19 . "</li><br><li>" . questday20 . " </li><br><li>" . questday21 . " </li><br>";
                    break;
            }
